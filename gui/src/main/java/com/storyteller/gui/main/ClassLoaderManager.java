...
public class ClassLoaderManager {
	...
	public boolean produceJar(String classPath, String modelName) {
		try {
			...
			String absolutePath = getClass().getResource("/"+jarName).getFile().substring(1);
			System.out.println("com.storyteller_f.sql_query.query 库位置："+absolutePath);
			ProcessBuilder javac = new ProcessBuilder().command("javac", "-classpath",
					absolutePath, classPath + "\\*.java", "-d",
					"output", "-encoding", "utf-8");
			System.out.println(javac.command());
